...
public class WeaponShotgun : MonoBehaviour
{
    ...
    void Shoot()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            GameObject bullet = Instantiate(bulletPrefab, firePoint[i].position, Quaternion.identity); // firepoint.rotation
            Rigidbody2D rb = bullet.GetComponent<Rigidbody2D>();
            rb.AddForce(firePoint[i].right * Speed, ForceMode2D.Impulse);
